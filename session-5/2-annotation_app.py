...
def get_annotations_file_path(file_path):
    """Return annotation CSV path for a given source CSV."""
    csv_path = Path(file_path).resolve()
    return csv_path.with_name(f"{csv_path.stem}-annotations.csv")

def get_labels_file_path(file_path):
    """Return labels TXT path for a given source CSV."""
    csv_path = Path(file_path).resolve()
    return csv_path.with_name(f"{csv_path.stem}-labels.txt")

def load_csv_files():
    """Load all CSV files ending with 'request_response.csv' from the data directory and its subdirectories,
    sorted by modification time (newest first)"""
    data_dir = Path("data")
    if not data_dir.exists():
        return []
    
    csv_files = list(data_dir.rglob("*request_response.csv"))
...
def load_annotations(file_path):
    ...
    annotations_file = get_annotations_file_path(file_path)
...
def load_binary_labels(file_path):
    ...
    labels_file = get_labels_file_path(file_path)
...
def initialize_annotations_with_labels(annotations_df, df_length, labels):
    ...
    if annotations_df.empty:
        annotations_df = pd.DataFrame({
            ...
            'notes': pd.Series([''] * df_length, dtype='object')
        })
    ...
    if 'notes' in annotations_df.columns:
        annotations_df['notes'] = annotations_df['notes'].astype('object')
...
def main():
    ...
    file_key = hashlib.md5(selected_file.encode("utf-8")).hexdigest()[:10]

    # Reset navigation state when the file changes
    if st.session_state.get("current_file") != selected_file:
        st.session_state.current_file = selected_file
        st.session_state.record_index = 0
        st.session_state.record_selectbox = 0
    ...
    with col2:
        ...
        if "record_selectbox" not in st.session_state:
            st.session_state.record_selectbox = st.session_state.record_index
        
        # Sync selectbox widget state with session state
        if st.session_state.record_selectbox != st.session_state.record_index:
            st.session_state.record_selectbox = st.session_state.record_index
        ...
        new_index = st.selectbox(
            "Select Record:",
            range(len(df)),
            index=st.session_state.record_selectbox,
        ...
        if new_index != st.session_state.record_index:
            ...
            st.session_state.record_selectbox = new_index
    ...
    def go_to_previous():
        new_index = max(0, st.session_state.record_index - 1)
        st.session_state.record_index = new_index
        st.session_state.record_selectbox = new_index
    
    def go_to_next():
        new_index = min(len(df) - 1, st.session_state.record_index + 1)
        st.session_state.record_index = new_index
        st.session_state.record_selectbox = new_index
    
    def go_to_last():
        last_index = len(df) - 1
        st.session_state.record_index = last_index
        st.session_state.record_selectbox = last_index
    
    # Navigation buttons - moved above the content
    nav_col1, nav_col2, nav_col3, nav_col4, nav_col5 = st.columns([1, 1, 1, 1, 1])
    
    with nav_col1:
        st.button("⏮️ First", disabled=(record_index == 0), key="btn_first", on_click=go_to_first)
    
    with nav_col2:
        st.button("⏪ Previous", disabled=(record_index == 0), key="btn_previous", on_click=go_to_previous)
    
    with nav_col3:
        st.metric("Progress", f"{record_index + 1}/{len(df)}")
    
    with nav_col4:
        st.button("Next ⏩", disabled=(record_index >= len(df) - 1), key="btn_next", on_click=go_to_next)
    
    with nav_col5:
        st.button("Last ⏭️", disabled=(record_index >= len(df) - 1), key="btn_last", on_click=go_to_last)
    ...
    if record_index < len(df):
        ...
        with left_col:
            ...
            st.text_area(
                ...
                height=400,
                key=f"user_input_{file_key}_{record_index}",
                help="Read-only view of the user input"
            )
        
        with middle_col:
            st.subheader("💬 LLM Response")
            llm_output = current_record.get('LLM Output', '')
            st.text_area(
                "LLM Output Content:",
                value=str(llm_output),
                height=600,
                key=f"llm_output_{file_key}_{record_index}",
                help="Read-only view of the LLM output"
            )
        ...
        with right_col:
            ...
            notes = st.text_area(
                ...
                key=f"notes_{file_key}_{record_index}",
            ...
            if notes != current_notes:
                ...
                st.toast("Notes saved automatically!", icon="✅")
                ...
                with checkbox_col1:
                    for label in left_labels:
                        ...
                        new_value = st.checkbox(
                            label,
                            value=current_value,
                            key=f"label_{file_key}_{label}_{record_index}"
                        )
            ...
            if binary_labels:
                ...
                for i, (label, count) in enumerate(label_counts):
                    ...
                    with label_display_col2:
                        ...
                        if st.button("🗑️", key=f"remove_{file_key}_{original_index}", help=f"Remove '{label}' label"):
        ...
        with label_col2:
            ...
            new_label = st.text_input(
                "New Label Name:",
                placeholder="e.g., Good, Relevant, Accurate",
                key=f"new_label_{file_key}"
            )
            if st.button("➕ Add Label", key=f"add_label_{file_key}"):
        ...
        if st.button("📥 Download Annotations", key=f"download_annotations_{file_key}"):
            # Create a dataframe with the specified columns plus binary labels
            export_data = {
                'user input': df['User Input'],
                'llm output': df['LLM Output'],
